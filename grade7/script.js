...
const ADDED_STUDENTS_KEY = "added_students_g7";
let optionButtons = [];
...
function renderQuestion() {
    ...
    if (current >= questions.length) {
        ...
        repeatBtn.className = "option-btn"; // Using option-btn for consistent styling in the container
...
function recordAndOfferDownload() {
    ...
    submitToGoogleSheets("7", studentName, score, questions.length);
    ...
    downloadBtn.onclick = () => {
        ...
        const ws = XLSX.utils.json_to_sheet(data);
        XLSX.utils.book_append_sheet(wb, ws, "grade7-result");
        XLSX.writeFile(wb, `${studentName}_Grade7_Result.xlsx`);
    };
