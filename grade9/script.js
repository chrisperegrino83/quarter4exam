    ...
    try {
        ...
        if (txt) {
            ...
            if (parsed && parsed.length) {
                ...
                const groupedQuestions = groupDependentQuestions(questions, 9);
...
function recordAndOfferDownload() {
    ...
    submitToGoogleSheets("9", studentName, score, questions.length);
    ...
    downloadBtn.onclick = () => {
        ...
        const ws = XLSX.utils.json_to_sheet(data);
        XLSX.utils.book_append_sheet(wb, ws, "grade9-result");
        XLSX.writeFile(wb, `${studentName}_Grade9_Result.xlsx`);
    };
