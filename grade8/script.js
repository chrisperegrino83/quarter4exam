...
let feedbackEl = null;
let nextBtn = null;
const SCORE_KEY = "g8_scores";
const ADDED_STUDENTS_KEY = "added_students_g8";
let optionButtons = [];
...
function showNameForm() {
    questionEl.textContent = "Enter your name to start the 4th Quarter Grade 8 - VE Examination";
...
function renderQuestion() {
    ...
    if (current >= questions.length) {
        ...
        questionEl.innerHTML = (studentName ? (studentName + ", ") : "") + 
            "Tapos na ang pagsagot mo sa 4th Quarter Grade 8 - VE Examination.<br>" +
    ...
    try {
        ...
        if (txt) {
            ...
            if (parsed && parsed.length) {
                ...
                const groupedQuestions = groupDependentQuestions(questions, 8);
...
function recordAndOfferDownload() {
    ...
    submitToGoogleSheets("8", studentName, score, questions.length);
    ...
    downloadBtn.onclick = () => {
        ...
        const ws = XLSX.utils.json_to_sheet(data);
        XLSX.utils.book_append_sheet(wb, ws, "grade8-result");
        XLSX.writeFile(wb, `${studentName}_Grade8_Result.xlsx`);
    };
